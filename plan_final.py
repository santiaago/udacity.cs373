...
def plan(road, lane_change_cost, init, goal): # Don't change the name of this function!
    ...
    #
    #
    #init matrix of costs (gval) and matrix of cells already visited (closed):
    gval = [[-1 for row in range(len(road[0]))] for col in range(len(road))]
    closed = [[0 for row in range(len(road[0]))] for col in range(len(road))]
    closed[init[0]][init[1]] = 1
    #initialize delta mouvements (only to the right and diagonals are allowed):
    delta = [[-1, 1 ], # go up diagonal
             [ 1, 1 ], # go down diagonal
             [ 0, 1 ]] # go right
    #initialize x and y to init position, set first cost at zero
    x = init[0]
    y = init[1]
    g = 0
    #open matrix will be used to handle the best cost to choose from
    open = [[g,x,y]]
    found = False
    resign = False
    while not found and not resign:
        if len(open)==0:
            ...
            #sort and reverse to have the lower cost vals at the end (so pop take the lower cost)
            open.sort()
            open.reverse()
            next = open.pop()
            #current vals to work with
            x = next[1]
            y = next[2]
            g = next[0]
            gval[x][y]=g
            #if you find the goal, then nothing else to do :-)
            if x == goal[0] and y == goal[1]:
                found = True
            else:
                #go throught all possible mouvements
                for i in range(len(delta)):
                    x2 = x + delta[i][0]
                    y2 = y + delta[i][1]
                    #make sure the car does not get out of the road
                    if x2 >=0 and x2 <len(road) and y2 >=0 and y2 < len(road[0]):
                        #make sure there are no obstacles and this coords were not already visited
                        if road[x2][y2]>0 and closed[x2][y2]==0:
                            if i == 2: # go right (straight)
                                g2 = g + 1./road[x2][y2]
                            else:#change lane, add lane change cost to the normal cost
                                g2 = g + lane_change_cost +  1./road[x2][y2]
                            #append mouvement and cost
                            open.append([g2,x2,y2])
                            closed[x][y]=1
    #return the cost of the goal coordinates :-)
    cost = gval[goal[0]][goal[1]]
    return cost
