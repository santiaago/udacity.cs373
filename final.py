...
class robot:
    ...
    def move(self, motion):
        ...
        if abs(turning_angle) <0.0001:
            ...
            self.x = cx + sin(self.orientation+turning_angle)*R
            self.y = cy - cos(self.orientation+turning_angle)*R
    ...
    def sense(self, noise =1): #do not change the name of this function
        ...
        for mark in landmarks:
            deltaY = mark[0] - self.y
            deltaX = mark[1] - self.x
            bearing = atan2(deltaY,deltaX)-self.orientation
            if bearing <0.:
                bearing = (2*pi) + bearing
            if noise == 1:
                bearing = ((2*pi)+ bearing + random.gauss(0, self.bearing_noise))%(2*pi)
            #elif bearing <0.0001:
            #    bearing = 0.  
            Z.append(bearing)
