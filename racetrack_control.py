...
############## ONLY ADD / MODIFY CODE BELOW THIS LINE ####################
    ...
    def cte(self, radius):
        ...
        if x > radius and x < (radius + 2*radius):
            if y > radius:#up: going right
                cte = y - (2*radius)
            else:#down: going left
                if y>0:
                    cte = -y
                else:
                    cte = abs(y)
            ...
            if x <radius:#left side
                distance = sqrt( (pow( x-C1x ,2)) + (pow(y - C1y ,2) )  )
            else:#right side
                distance = sqrt( (pow( x-C2x ,2)) + (pow(y - C2y ,2) )  )
            cte = distance - radius
        return cte
...
def run(params, radius, printflag = False):
    ...
    for i in range(N*2):
        ...
        if printflag:
            print str(myrobot)+ " cte:"+ str(crosstrack_error) + " steer:"+str(steer)
    return err / float(N)
