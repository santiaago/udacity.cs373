...
def slam(data, N, num_landmarks, motion_noise, measurement_noise):
    ...
    #
    #
    # 2 because we are modeling with x and y
    dimention = 2 * (N + num_landmarks)
    #init Omega
    Omega = matrix()
    Omega.zero(dimention, dimention)
    #initial state
    Omega.value[0][0] = 1.
    Omega.value[1][1] = 1
    #init XI
    ...
    for i in range(len(data)):
        ...
        for j in range(len(measurement)):
            # m is index of the landmarks coordinates
            # times 2 because we have X and Y coordinates
            # + N because we jump directly to the landmark coords
            #measurement[j][0] is the current landmark
            m = 2 * (N + measurement[j][0])

            for k in range(2):
                # update set plus and minus in the Omega matrix
                # k handles x and y coords
                # n handles the x coordinate of each Xo to Xn and L0 to Lm

                #Xn diagonal
                Omega.value[n+k][n+k] = Omega.value[n+k][n+k] + 1. / measurement_noise
                #landmark diagonal
                Omega.value[m+k][m+k] = Omega.value[m+k][m+k] + 1. / measurement_noise
                #Landmark on row Xn
                Omega.value[n+k][m+k] = Omega.value[n+k][m+k] - 1. / measurement_noise
                #Xn on row Landmark
                Omega.value[m+k][n+k] = Omega.value[m+k][n+k] - 1. / measurement_noise
                #update set plus and minus in the Xi vector
                #Xn x and y 
                Xi.value[n+k][0] = Xi.value[n+k][0] - measurement[j][1+k] / measurement_noise
                #landmark x and y
                Xi.value[m+k][0] = Xi.value[m+k][0] + measurement[j][1+k] / measurement_noise
        #motion main diagonal
        for l in range(4):
            Omega.value[n+l][n+l] = Omega.value[n+l][n+l] + 1. / motion_noise
        #motion
        for l in range(2):
            Omega.value[n+l][n+l+2] = Omega.value[n+l][n+l+2] - 1. / motion_noise
            Omega.value[n+l+2][n+l] = Omega.value[n+l+2][n+l] - 1. / motion_noise
            
            Xi.value[n+l][0] = Xi.value[n+l][0] - motion[l] / motion_noise
            Xi.value[n+l+2][0] = Xi.value[n+l+2][0] + motion[l] / motion_noise
