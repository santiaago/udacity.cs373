...
def plan(road, lane_change_cost, init, goal): # Don't change the name of this function!
    ...
    #for r in road:
    #    print r
    #print 'lane change cost: '+str(lane_change_cost)
    #print 'init: '+str(init)
    #print 'goal: '+str(goal)
    #init matrix:
    ...
    while not found and not resign:
        ...
        if len(open)==0:
            ...
            g = next[0]
            gval[x][y]=g
            #print "working with x="+str(x)+" y="+str(y)+" g="+str(g)
                ...
                for i in range(len(delta)):
                    ...
                    if x2 >=0 and x2 <len(road) and y2 >=0 and y2 < len(road[0]):
                        ...
                        if road[x2][y2]>0 and closed[x2][y2]==0:
                            if i == 2: # go left(straight)
                                #print 'g:'+str(g)
                                #print 'road[x2][y2]'+str(road[x2][y2])
                                g2 = g + 1./road[x2][y2]
                            else:#change lane
                                #print 'g:'+str(g)
                                #print 'change lane:'+str(lane_change_cost)
                                g2 = g + lane_change_cost +  1./road[x2][y2]
                            #print "appending: "+ str([g2,x2,y2])
