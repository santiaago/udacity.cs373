...
def smooth(path, weight_data = 0.1, weight_smooth = 0.1, tolerance = 0.00001):
    ...
    # Enter code here
    #
    #deep copy of path into new path
    ...
    while change >= tolerance:
        ...
        for i in range(len(path)):
            for j in range(len(path[0])):
                ...
                newpath[i][j] = newpath[i][j] + beta  * (newpath[(i+1)%len(path)][j]+newpath[(i-1)%len(path)][j]-(2.0*newpath[i][j]))
                #if(i==len(path)-1):#last case boundary
                #    newpath[i][j] = newpath[i][j] + beta  * (newpath[0][j]+newpath[i-1][j]-(2.0*newpath[i][j]))
                #elif(i==0):#first case boundary
                #    newpath[i][j] = newpath[i][j] + beta  * (newpath[i+1][j]+newpath[len(path)-1][j]-(2.0*newpath[i][j]))
                #else:
